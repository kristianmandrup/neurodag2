module AjaxfulRating # :nodoc:
  ...
  def static_stars(stars)
    ...
    def build_star(rateable, user, i)
      ...
      # logger.debug("BUILD STAR: #{i}")
      # logger.debug(rateable.to_yaml)
    ...
    def extract_options(rateable, *args)
      ...
      url = "rate_#{rateable.class.name.downcase}_path"
      options[:remote_options][:url] = get_url(url, rateable)
      options[:html].reverse_merge!(:class => "#{options[:class]} #{options[:small_star_class] if options[:small_stars]}")
            
      user
    end
    
    def get_url(url, rateable)
      if respond_to?(url)
        send url, rateable
      else
